...
$sql = "SELECT * from teacher_registration";
 ...
 while($row = mysqli_fetch_array($check))
 {
	 ...
	 echo "<td><button class=btn1><a href=admin_teacher_update.php?email=".$row['email'].">Update</a></button></td>";
	 echo  "<td><div class=dropdown>
                                ...
                                <a href=delete_process_teacher.php?email=".$row['email']."><button class=dropdown-item style=color:#000;>Delete</button></a>
                                <a href=block_process_teacher.php?email=".$row['email']."><button class=dropdown-item style=color:#000;>Block</button></a>                               
                                </div>
                              </div>
							 </td>";
	...
	$("body").on("click", "#btnExport", function () {
            html2canvas($('#tblUsers')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("registerd_teacher.pdf");
                }
            });
        });
    </script>
