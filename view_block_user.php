...
$sql = "SELECT * FROM block_user";
 ...
 while($row = mysqli_fetch_array($check))
 {
	 ...
	 echo  "<td><button class=btn2><a href=unblock_process_user.php?email=".$row['email'].">Unblock</a></td>";
	...
	$("body").on("click", "#btnExport", function () {
            html2canvas($('#tblUsers')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("block_student.pdf");
                }
            });
        });
    </script>
